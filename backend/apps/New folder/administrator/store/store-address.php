<?php
	...
	$aColumns = array(
		...
		'address.updated',
		'address.id');
	$sIndexColumn = "address.id";
	
	$sTable = "address
	LEFT JOIN countries ON address.country = countries.id
	LEFT JOIN cities ON address.city = cities.id
	LEFT JOIN districts ON address.district = districts.id
	LEFT JOIN subdistricts ON address.subdistrict = subdistricts.id
	...
	for ( $i=0 ; $i<count($aColumns) ; $i++ ){
		if ( $_REQUEST['bSearchable_'.$i] == "true" && $_REQUEST['search_'.$i] != '' ){
			if ( $sWhere == "" ){
				$sWhere = "WHERE ";
			}else{
				$sWhere .= " AND ";
			}
			$sWhere .= $aColumns[$i]." LIKE '%".$dbc->Escape_String($_REQUEST['search_'.$i])."%' ";
		}
	}
