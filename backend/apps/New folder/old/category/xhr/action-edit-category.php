<?php
	...
	if($dbc->HasRecord("categories","name = '".$_REQUEST['txtName']."' AND id !=".$_REQUEST['txtID'])){
		echo json_encode(array(
			'success'=>false,
			'msg'=>'Categories Name is already exist.'
		));
	}else{
		$cat = $dbc->GetRecord("categories","*","id='".$_REQUEST['txtID']."'");
		$json = json_decode($cat['media'],true);
		$youtube = array();
...
//			'advertisement' => $_REQUEST['txtads']
		...
		if($dbc->Update("categories",$data,"id=".$_REQUEST['txtID'])){
			$id = $dbc->GetID();
			echo json_encode(array(
				'success'=>true
			));
