...
<div class="col-md-12 breadcrumb"><font size="2"><a href="?app=<?php echo $_REQUEST['app'];?>">ร้านตัวแทนจำหน่าย</a></font></div>
    ...
    <div><!-- Nav tabs -->
            	...
            	<div class="panel panel-default">
                  <div class="panel-heading">
                    ...
                    <!--<div align="right">-->
                        <button type="button"  class="btn btn-danger btn-sm pull-right but" onclick="fn.navigate('add')">เพิ่ม ร้านตัวแทนจำหน่าย</button>
                    <!--</div>-->
                  </div>
                  <div class="panel-body nopad">
                        ...
                        <table class="table table-bordered rwd-table">
                            <thead>
                                <tr>
                                    ...
                                    <th class="text-center">ภูมิภาพ</th>
                    ...
                    <?php 
                    $sql = $dbc->Query("SELECT * FROM store");
                    $i=1;
                    ...
                    {	
                        ...
                        <tr>
                            ...
                            <td data-th="Detail" class="text-right"><?php echo explode("|",base64_decode($row['detail']))[0];?></td>
                            <td data-th="Detail" class="text-right">
                            <?php 
							$branch = $dbc->GetRecord("branch","*","id = '".$row['branch']."'");
							$sec = $dbc->GetRecord("sector","*","id = '".$branch['sector']."'");
							echo explode("|",$sec['name'])[0];
							?>
                            </td>
                            <td data-th="Detail" class="text-right">
                            <?php 
							
							echo explode("|",$branch['name'])[0];
							?>
                            </td>
                            <?php /*?><td data-th="Date Create" class="text-center"><?php 
                            ...
                            <td data-th="Action" class="text-center">
                                ...
                                <button type="button" class="buticonRem" onClick="fn.app.store.dialog_delete('<?php echo $row['id'];?>',this)"><i class="fa fa-times" style="font-size:14px;"></i></button>
                            </td>
                            <td data-th="Status." class="text-center">
                            <?php if($row['status']==1)
                            {
                                ?><div class="switch">
                                    <input id="cmn-toggle-<?php echo $i;?>" class="cmn-toggle cmn-toggle-round" checked type="checkbox" onClick="fn.app.store.activ('<?php echo $row['id'];?>',this)">
                            ...
                            {
                                ?><div class="switch">
                                    <input id="cmn-toggle-<?php echo $i;?>" class="cmn-toggle cmn-toggle-round" type="checkbox" onClick="fn.app.store.activ('<?php echo $row['id'];?>',this)">
                                    <label for="cmn-toggle-<?php echo $i;?>"></label>
                                </div><?php
                            }?>
