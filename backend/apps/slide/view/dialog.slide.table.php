...
<div class="col-md-12 breadcrumb"><font size="2"><a href="?app=<?php echo $_REQUEST['app'];?>">ภาพสไลด์</a></font></div>
    ...
    <div><!-- Nav tabs -->
            	...
            	<div class="panel panel-default">
                  <div class="panel-heading">
                    ...
                    <div align="right">
                        <!--
						<button type="button"  class="btn btn-success btn-sm  but" onclick="fn.app.slide.save_sortOrder()">บันทึกลำดับ</button>
						-->
                        <button type="button"  class="btn btn-danger btn-sm  but" onclick="fn.navigate('add')">เพิ่มภาพสไลด์</button>
                    </div>
                  </div>
                  <div class="panel-body nopad">
                    <div class=" nopad"><!--table-responsive-->
                        <table id="slides" class="table table-bordered rwd-table">
                            <thead>
                                <tr>
                                    <th class="text-center">จัดลำดับ</th>
                                    <th class="text-center">ลำดับ</th>
                                    <th class="text-center no-sort">ภาพ</th>
                    ...
                    <?php 
                    $sql = $dbc->Query("SELECT * FROM slide_photo order by sort asc");
                    $i=1;
                    while($row = $dbc->Fetch($sql))
                    {	
                        ?>
                        <tr id="<?php echo $row['id'];?>">
                            <td class="text-center" valign="middle"><img src="../../../../libs/upload/arrows.png" width="20"></td>
                            <td data-th="No." align="center"><?php echo $i;?></td>
                            <!--
							<td data-th="Image" class="text-right"><img src="../libs<?php //echo $row['photo'];?>" width="100"></td>
							-->
							<td data-th="Image" class="text-right"><img src="../../../../libs/<?php echo $row['photo'];?>" width="100"></td>
                            <td data-th="Date Create" class="text-center"><?php 
							$aMonth = array('January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December');
							$y = substr($row['created'],0,4);
							$m = substr($row['created'],5,2);
							$d = substr($row['created'],8,2);
							echo $d.' / ';
							switch($m)
							{
                            ...
                            <td data-th="Action" class="text-center">
                                ...
                                <button type="button" class="buticonRem" onClick="fn.app.slide.dialog_delete('<?php echo $row['id'];?>',this)"><i class="fa fa-times" style="font-size:14px;"></i></button>
                            </td>
                            <td data-th="Status." class="text-center">
                            <?php if($row['status']==1)
                            {
                                ?><div class="switch">
                                    <input id="cmn-toggle-<?php echo $i;?>" class="cmn-toggle cmn-toggle-round" checked type="checkbox" onClick="fn.app.slide.activ('<?php echo $row['id'];?>',this)">
                            ...
                            {
                                ?><div class="switch">
                                    <input id="cmn-toggle-<?php echo $i;?>" class="cmn-toggle cmn-toggle-round" type="checkbox" onClick="fn.app.slide.activ('<?php echo $row['id'];?>',this)">
                                    <label for="cmn-toggle-<?php echo $i;?>"></label>
                                </div><?php
                            }?>
